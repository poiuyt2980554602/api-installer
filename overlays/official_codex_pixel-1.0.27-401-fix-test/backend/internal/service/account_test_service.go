...
type AccountTestService struct {
	...
	proxyAffinityService      *ProxyAffinityService
}
...
func (s *AccountTestService) SetProxyAffinityService(proxyAffinityService *ProxyAffinityService) {
	s.proxyAffinityService = proxyAffinityService
}
...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string) error {
	...
	if s.proxyAffinityService != nil {
		boundAccount, _, bindErr := s.proxyAffinityService.EnsurePreValidationBinding(ctx, accountID)
		if bindErr != nil {
			if errors.Is(bindErr, ErrProxyAffinityNoProxyAvailable) {
				return s.sendErrorAndEnd(c, "Proxy Affinity requires a proxy before validation, but no assignable proxy is available")
			}
			return s.sendErrorAndEnd(c, fmt.Sprintf("Proxy Affinity pre-validation binding failed: %s", bindErr.Error()))
		}
		if boundAccount != nil {
	...
	defer func() {
		...
		result := ProxyAffinityValidationResult{Success: testErr == nil}
		if testErr != nil {
			result.Error = testErr.Error()
		}
		if err := s.proxyAffinityService.HandleValidationResult(ctx, accountID, result); err != nil {
			slog.Warn("proxy affinity validation result update failed", "account_id", accountID, "error", err)
		}
	}()
